import { markdownToDiary, markdownToGuestBook, DiaryEntry, GuestBookEntry } from './utils';

// Vite의 import.meta.glob을 사용하여 모든 마크다운 파일을 동적으로 로드
const markdownModules = import.meta.glob('./*.md', { as: 'raw', eager: true });

/**
 * 모든 마크다운 파일을 로드하여 DiaryEntry 배열로 변환
 */
export function loadDiaries(): DiaryEntry[] {
  ...
  for (const [path, content] of Object.entries(markdownModules)) {
    if (typeof content === 'string') {
      ...
      const diary = markdownToDiary(content, filename);
      diaries.push(diary);
    }
  }
